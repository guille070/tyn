    ...
    function get_author() {
        ...
        if ($this->is_review === false) {
            if (td_util::get_option('tds_p_show_author_name') != 'hide') {
				...
				//firma
				if ((intval(get_post_meta($this->post->ID, 'post_signature', true )) > 0)) {
                $buffy .= '<a href="' . get_author_posts_url($this->post->post_author) . '">' . get_the_author_meta('display_name', $this->post->post_author) . '</a>' ;
				}else{
                $buffy .= '<a href="' . get_author_posts_url(42) . '">' . get_the_author_meta('display_name', 42) . '</a>' ;
					//if ((intval(get_the_author_meta('display_name', $this->post->post_author))) != 42) {
					//	$the_post = array();
					//	$the_post['ID'] = $this->post->ID;
					//	$the_post['post_author'] = 42;
					//	wp_insert_post( $the_post );
						//update_post_meta($this->post->ID, 'post_author', 42, true);
					//}
				}
				//end firma
                    ...
                    $buffy .= '<img width="' . $td_temp_image_url[1] . '" height="' . $td_temp_image_url[2] . '" class="entry-thumb" src="' . $td_temp_image_url[0] . '" ' . $attachment_alt . $attachment_title . '/>';
                        ...
                        if (get_post_format($this->post->ID) == 'video') {
                            ...
                            if ($use_small_post_format_icon_size === true) {
                                ...
                                $buffy .= '<span class="td-video-play-ico"><img width="60" class="td-retina" src="' . td_global::$get_template_directory_uri . '/images/icons/ico-video-large.png' . '" alt="video"/></span>';
                            }
                        } // end on video if
