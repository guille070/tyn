...
class GP_Plugin_Updater
{
	 ...
	 */
	public function __construct( $_api_url, $_plugin_file, $_api_data = null, $_options = array() )
	{
		$this->api_url = $_api_url;
		$this->base_file = $_plugin_file;
		$this->options = $this->parse_options($_options);
		...
		add_action( "after_plugin_row_$plugin_name", array($this, 'plugin_row_messages'), 20, 3 );
		add_filter( 'site_transient_update_plugins', array($this, 'catch_update_notifications') );

		// create a child instance of the GP_EDD_SL_Plugin_Updater, which
		// will do most of the work
		$this->plugin_updater = new GP_EDD_SL_Plugin_Updater($_api_url, $_plugin_file, $_api_data);
	}
	...
	function plugin_row_messages($plugin_file, $plugin_data, $status)
	{
		...
		// automatic updates
		$activate_link = $this->create_link( $this->options['activate_url'], __('Activate your API key') );
		$link_to_plugin = $this->create_link( $this->options['info_url'], $this->options['plugin_name'] );
		$message = sprintf(
						'<strong style="color:#a00">%s:</strong> %s %s %s.',
						__('Important'),
						$activate_link,
						__('to enable automatic updates for'),
						$link_to_plugin
					);

		printf ( '<tr class="plugin-update-tr active">
					<td>&nbsp;</td>
					<td colspan="2" style="border-left: 0 none;">%s</td>
				  </tr>',
				 $message
			   );
	}
	...
	function display_admin_notices($message, $notice_level = 'error')
	{
		...
		if ( !empty($message) ) {
			printf ( '<div class="notice notice-%s is-dismissible gp_plugin_updater_notice"><p>%s</p></div>',
					 $notice_level,
					 $message );
